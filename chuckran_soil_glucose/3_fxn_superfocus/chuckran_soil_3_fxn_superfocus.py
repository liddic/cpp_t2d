# # # # # # # # # # # # # # # #
# # Run Superfocus fxn annotations from reads - for Chuckran et al soil glucose metagenomics
# # # # # # # # # # # # # # # #
...
READDIR = '/scratch/pawsey1216/cliddicoat/chuckran_soil_glucose/2_clean_fastp'

# set working directory
workDir = '/scratch/pawsey1216/cliddicoat/chuckran_soil_glucose/3_fxn_superfocus'
...
table = pd.read_csv('/scratch/pawsey1216/cliddicoat/chuckran_soil_glucose/1_meta_raw/chuckran_glucose_combined_reads.tsv.txt', sep='\t')
...
for i in range(n):
    ...
    fh.writelines("#SBATCH --cpus-per-task=64\n")
    ...
    fh.writelines("superfocus -t 64 -q %s -dir %s -a diamond -db DB_100 --alternate_directory %s\n" % (qc_read_r1,OUTDIR,path_to_db_dir))

# # # # # # # # # # # # # # # #
# # Run Superfocus fxn annotations from reads - for Sun & Badgley post-mining forest restoration metagenomics
# # # # # # # # # # # # # # # #
...
READDIR = '/scratch/pawsey1216/cliddicoat/sunbad_resto/2_clean_mgrast'

# set working directory
workDir = '/scratch/pawsey1216/cliddicoat/sunbad_resto/3_fxn_superfocus'
...
table = pd.read_csv('/scratch/pawsey1216/cliddicoat/sunbad_resto/2_clean_mgrast/sunbad-resto-mgp16379_metadata.tsv.txt', sep='\t')
...
for i in range(n):
    ...
    qc_read_r1 = os.path.join(READDIR, "%s.299.screen.passed.fna" % list(samples)[i])
    ...
    fh.writelines("superfocus -t 48 -q %s -dir %s -a diamond -db DB_100 --alternate_directory %s\n" % (qc_read_r1,OUTDIR,path_to_db_dir))
